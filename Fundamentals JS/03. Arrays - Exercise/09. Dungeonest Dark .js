function dungeonestDark(array) {
  ...
  for (let index = 0; index < allRooms.length; index++) {
    // Правим цикъла да минем през всички стаи, освен ако не умрем
    let room = allRooms[index].split(" "); // Разделяме масива с всички стаи на отделни, всеки път когато минаваме през цикъла
    level++;

    if (room[0] === "potion") {
      // Проверяваме какво има в стаята чрез нулевия индекс
      let takenPotion = Number(room[1]); // Текущата кръв, която намираме като число
      if (health <= 100) {
        // Ако кръвта ни до момента е по-малко от 100, можем да я вземем
        if (health + takenPotion >= 100) {
          // Ако сбора на текущата и взетата е повече от 100, не можем да вземем цялата
          ...
          // Ако кръвта ни е по-малко от 100 и можем да вземем цялата
      ...
      if (health > 0) {
        // Ако кръвта ни е повече от 0(не сме умрели)
        console.log(`You slayed ${currentMonster}.`);
      } else {
        // Ако кръвта ни е станала 0 или по-малко сме умрели
