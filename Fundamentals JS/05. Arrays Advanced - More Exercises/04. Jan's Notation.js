function jansNotation(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    // Правим цикъл, с който обикаляме по масива за числа и оператори
    ...
    if (numbers.length >= 2 && operators.length === 1) {
      // Ако в някакъв момент имаме 2 или повече числа и оператор
      while (operators.length > 0) {
        // Ако няма повече оператори, печатаме числата към момента
        ...
        switch (
          operator // Проверяваме какъв е оператора
        ) {
          case "+":
  ...
  if (numbers.length - operators.length > 1) {
    // Ако след добавянето на всичко имаме повече числа, отколкото оператори
  ...
  if (operators.length - numbers.length >= 0) {
    // Ако след добавянето на всичко имаме повече оператори и недостатъчно числа
